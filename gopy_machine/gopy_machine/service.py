	...
	def __getitem__(self, key):
		if isinstance(key, slice):
			if key.step == None or key.step == 1:
				...
				return Slice_Ptr_casemanager_Case(handle=_gopy_machine.Slice_Ptr_casemanager_Case_subslice(self.handle, st, ed))
	...
	def __getitem__(self, key):
		if isinstance(key, slice):
			if key.step == None or key.step == 1:
				...
				return Slice_Ptr_model_PathNode(handle=_gopy_machine.Slice_Ptr_model_PathNode_subslice(self.handle, st, ed))
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			if key < 0:
				key += len(self)
	...
	def __getitem__(self, key):
		if isinstance(key, slice):
			if key.step == None or key.step == 1:
				...
				return Slice_Ptr_utils_RuleSpec(handle=_gopy_machine.Slice_Ptr_utils_RuleSpec_subslice(self.handle, st, ed))
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			if key < 0:
				key += len(self)
...
class CaseManager(go.GoClass):
	...
	def SubmitCase(self, ctx, bsn, serviceType, law, parameters, claimedResult, approvedClaimsOnly):
		...
		"""
		return go.uuid_UUID(handle=_gopy_machine.service_CaseManager_SubmitCase(self.handle, ctx.handle, bsn, serviceType, law, parameters.handle, claimedResult.handle, approvedClaimsOnly))
	def CompleteManualReview(self, ctx, caseID, verifierID, approved, reason, overrideResult):
		...
		"""
		return _gopy_machine.service_CaseManager_CompleteManualReview(self.handle, ctx.handle, caseID.handle, verifierID, approved, reason, overrideResult.handle)
	...
	def GetCase(self, bsn, serviceType, law):
		...
		"""
		return go.Ptr_casemanager_Case(handle=_gopy_machine.service_CaseManager_GetCase(self.handle, bsn, serviceType, law))
	def GetCaseByID(self, ctx, id):
		"""GetCaseByID(object ctx, []int id) object, str
		
		GetCaseByID gets a case by ID
		"""
		return go.Ptr_casemanager_Case(handle=_gopy_machine.service_CaseManager_GetCaseByID(self.handle, ctx.handle, id.handle))
	def GetCasesByStatus(self, serviceType, status):
		...
		"""
		return Slice_Ptr_casemanager_Case(handle=_gopy_machine.service_CaseManager_GetCasesByStatus(self.handle, serviceType, status))
	def GetCasesByLaw(self, law, serviceType):
		...
		"""
		return Slice_Ptr_casemanager_Case(handle=_gopy_machine.service_CaseManager_GetCasesByLaw(self.handle, law, serviceType))
	def GetEventsByUUID(self, caseID):
		...
		"""
		return Slice_Ptr_model_Event(handle=_gopy_machine.service_CaseManager_GetEventsByUUID(self.handle, caseID.handle))
...
class ClaimManager(go.GoClass):
	...
	def SubmitClaim(self, ctx, svc, key, newValue, reason, claimant, law, bsn, caseID, oldValue, evidencePath, autoApprove):
		...
		"""
		return _gopy_machine.service_ClaimManager_SubmitClaim(self.handle, ctx.handle, svc, key, newValue.handle, reason, claimant, law, bsn, caseID.handle, oldValue.handle, evidencePath, autoApprove)
	def ApproveClaim(self, ctx, claimID, verifiedBy, verifiedValue):
		...
		"""
		return _gopy_machine.service_ClaimManager_ApproveClaim(self.handle, ctx.handle, claimID, verifiedBy, verifiedValue.handle)
	def RejectClaim(self, ctx, claimID, rejectedBy, rejectionReason):
		...
		"""
		return _gopy_machine.service_ClaimManager_RejectClaim(self.handle, ctx.handle, claimID, rejectedBy, rejectionReason)
	...
	def GetClaimsByService(self, svc, approved, includeRejected):
		...
		"""
		return Slice_Ptr_model_Claim(handle=_gopy_machine.service_ClaimManager_GetClaimsByService(self.handle, svc, approved, includeRejected))
	def GetClaimsByCase(self, caseID, approved, includeRejected):
		...
		"""
		return Slice_Ptr_model_Claim(handle=_gopy_machine.service_ClaimManager_GetClaimsByCase(self.handle, caseID.handle, approved, includeRejected))
	def GetClaimsByClaimant(self, claimant, approved, includeRejected):
		...
		"""
		return Slice_Ptr_model_Claim(handle=_gopy_machine.service_ClaimManager_GetClaimsByClaimant(self.handle, claimant, approved, includeRejected))
	def GetClaimsByBSN(self, bsn, approved, includeRejected):
		...
		"""
		return Slice_Ptr_model_Claim(handle=_gopy_machine.service_ClaimManager_GetClaimsByBSN(self.handle, bsn, approved, includeRejected))
	def GetClaimByBSNServiceLaw(self, bsn, svc, law, approved, includeRejected):
		...
		"""
		return Map_string_Ptr_model_Claim(handle=_gopy_machine.service_ClaimManager_GetClaimByBSNServiceLaw(self.handle, bsn, svc, law, approved, includeRejected))
...
class RuleService(go.GoClass):
	...
	def Evaluate(self, ctx, law, referenceDate, parameters, overwriteInput, requestedOutput, approved):
		...
		"""
		return go.Ptr_model_RuleResult(handle=_gopy_machine.service_RuleService_Evaluate(self.handle, ctx.handle, law, referenceDate, parameters.handle, overwriteInput.handle, requestedOutput, approved))
...
class Services(go.GoClass):
	...
	def EvaluateWithCtx(self, svc, law, parameters):
		...
		return go.Ptr_model_RuleResult(handle=_gopy_machine.service_Services_EvaluateWithCtx(self.handle, svc, law, parameters.handle))
	def Evaluate(self, ctx, svc, law, parameters, referenceDate, overwriteInput, requestedOutput, approved):
		...
		"""
		return go.Ptr_model_RuleResult(handle=_gopy_machine.service_Services_Evaluate(self.handle, ctx.handle, svc, law, parameters.handle, referenceDate, overwriteInput.handle, requestedOutput, approved))
