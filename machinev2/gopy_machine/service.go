...
#cgo LDFLAGS: "-L/usr/lib/x86_64-linux-gnu" "-lpython3.11" -ldl -lm
...
// --- generated code for package: service below: ---
...
func service_NewRuleService(serviceName *C.char, services CGoHandle) CGoHandle {
	_saved_thread := C.PyEval_SaveThread()
	defer C.PyEval_RestoreThread(_saved_thread)
	return handleFromPtr_Ptr_service_RuleService(service.NewRuleService(C.GoString(serviceName), ptrFromHandle_Ptr_service_Services(services)))

}

//export service_NewServices
func service_NewServices(referenceDate CGoHandle) CGoHandle {
	_saved_thread := C.PyEval_SaveThread()
	defer C.PyEval_RestoreThread(_saved_thread)
	return handleFromPtr_Ptr_service_Services(service.NewServices(*ptrFromHandle_time_Time(referenceDate)))

}

//export service_NewCaseManager
func service_NewCaseManager(services CGoHandle) CGoHandle {
	_saved_thread := C.PyEval_SaveThread()
	defer C.PyEval_RestoreThread(_saved_thread)
	return handleFromPtr_Ptr_service_CaseManager(service.NewCaseManager(ptrFromHandle_Ptr_service_Services(services)))
