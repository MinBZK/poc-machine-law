...
class ExistsConditionEvaluator:
    ...
    def _find_characteristic_item(self, characteristic_ref: list, context: NrmlRuleContext) -> str | None:
        ...
        """
        if isinstance(characteristic_ref, list) and len(characteristic_ref) >= 2:
            # The second element should be the characteristic item reference
            if isinstance(characteristic_ref[1], dict) and "$ref" in characteristic_ref[1]:
                return characteristic_ref[1]["$ref"]
        return None

    def _find_conditional_characteristic_source(self, characteristic_item_key: str, context: NrmlRuleContext) -> str | None:
