...
class ConditionalExpressionEvaluator:
    ...
    def evaluate(self, expression: dict[str, Any], context: NrmlRuleContext) -> EvaluationResult:
        ...
        if not condition_result.Success:
            return create_result(success=False, error="Failed to evaluate condition", source=self.__class__.__name__, sub_results=[condition_result])
    ...
    def resolve_expression_result(self, condition_result: EvaluationResult, context: NrmlRuleContext,
        ...
        if clause:
            ...
            return create_result(success=True, value=condition_result.Value, source=self.__class__.__name__, action=f"No {clause_key} clause found, returning condition result")
