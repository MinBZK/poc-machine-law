...
# =========================================================================
# AWIR Workflow Endpoints (Art. 19, 22, 24, 26a)
# =========================================================================


@router.post("/cases/{case_id}/ib-aanslag")
async def admin_ontvang_ib_aanslag(
    request: Request,
    case_id: str,
    vastgesteld_inkomen: int = Form(...),
    case_manager: CaseManagerInterface = Depends(get_case_manager),
):
    """
    Registreer ontvangst IB-aanslag (AWIR Art. 19 trigger).

    Dit triggert de definitieve vaststellingsprocedure:
    - Deadline: 6 maanden na IB-aanslag voor definitieve beschikking
    """
    try:
        case_manager.ontvang_ib_aanslag(
            case_id=case_id,
...
@router.post("/cases/{case_id}/definitief")
async def admin_stel_definitief_vast(
    request: Request,
    case_id: str,
    definitief_jaarbedrag: int = Form(...),
    case_manager: CaseManagerInterface = Depends(get_case_manager),
):
    """
    Stel toeslag definitief vast (AWIR Art. 19).

    Dit is de definitieve berekening op basis van het werkelijke inkomen.
    """
    try:
        case_manager.stel_definitief_vast(
            case_id=case_id,
...
@router.post("/cases/{case_id}/vereffen")
async def admin_vereffen(
    request: Request,
    case_id: str,
    case_manager: CaseManagerInterface = Depends(get_case_manager),
):
    """
    Voer vereffening uit (AWIR Art. 24, 26a).

    Berekent automatisch:
    - NABETALING: als definitief > totaal betaald
    - TERUGVORDERING: als definitief < totaal betaald (boven €116)
    - KWIJTGESCHOLDEN: als terugvordering < €116 (Art. 26a doelmatigheidsgrens)
    """
    try:
        result = case_manager.vereffen(case_id=case_id)
        return RedirectResponse(url=f"/admin/cases/{case_id}", status_code=303)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.post("/cases/{case_id}/verwerk-maand")
async def admin_verwerk_maand(
    request: Request,
    case_id: str,
    maand: int = Form(...),
    case_manager: CaseManagerInterface = Depends(get_case_manager),
    ...
    """
    Handmatig een maand verwerken (AWIR Art. 22).

    Voert herberekening + betaling uit voor een specifieke maand.
    """
    try:
        case = case_manager.get_case_by_id(case_id)

        # Herberekening via rules engine
        result = machine_service.evaluate(
            service="TOESLAGEN",
            law=case.law,
            parameters={"BSN": case.bsn},
        )
        maandbedrag = result.output.get("hoogte_toeslag", 0) // 12

        # Registreer herberekening
        case_manager.herbereken_maand(
            case_id=case_id,
            maand=maand,
        ...
        # Voer betaling uit (op basis van voorschot)
        case = case_manager.get_case_by_id(case_id)  # Refresh case
        betaald_bedrag = case.voorschot_maandbedrag or maandbedrag

        case_manager.betaal_maand(
            case_id=case_id,
            maand=maand,
